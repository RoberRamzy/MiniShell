...
{
    std::vector<Token> tokens;/*token is a struxt that holds to variables type & word*/
    std::istringstream stream(input);/*takes the stream of words*/
    std::string word;
    /*read untill it hits a whitespace*/
    while (stream >> word)
    {
        Token token;
        ...
        if (word == "|")
            ...
            {
                if (!tokens.empty())/*check if the tokens is empty*/
                {
                ...
                else/*the tokens is empty that means that the first word int the string is command*/
                {
                    token.type = TOKEN_COMMAND;
                }
            }
